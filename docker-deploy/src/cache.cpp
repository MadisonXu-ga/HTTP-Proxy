...
bool Cache::checkValidate(Request req, Response res, int request_id) {
  ...
  std::tm utc_now_tm = *std::gmtime(&now);
  std::tm * utc_now = &utc_now_tm;

  std::time_t response_receive_time = parseHttpResponseTime(res.getDate());
  std::tm utc_expire_tm;
  ...
  if (res.has_Maxage) {
    ...
    utc_expire_tm = *std::gmtime(&expires_time);
  }
  ...
  std::tm * utc_expire = &utc_expire_tm;

  // check the time
  if (expires_time < now) {
    // ID: in cache, but expired at EXPIREDTIME
    std::tm utc_expire_tm = *std::gmtime(&expires_time);
    std::tm * utc_expire = &utc_expire_tm;
    proxy_log << request_id << ": in cache, but expired at " << std::asctime(utc_expire);
    return false;
  }
